...
enum ObsidianExportMode: String, Codable, CaseIterable, Identifiable {
    ...
    /// in Soldo are always reflected in the vault.
    ...
    var managedFileWarning: String? {
        rebuildsWholeFile
            ? "Soldo riscrive questo file a ogni sincronizzazione: le modifiche fatte a mano dentro Obsidian andranno perse."
            : nil
    }
}

/// Everything about *where* and *how* Soldo writes into the vault.
...
struct ObsidianConfiguration: Codable, Equatable {
    ...
    var backupFileName: String = "soldo-backup.json"
...
enum ObsidianPath {
    ...
    static func ensureExtension(_ name: String, _ ext: String) -> String {
        ...
        guard !trimmed.isEmpty else { return "Soldo.\(ext)" }
