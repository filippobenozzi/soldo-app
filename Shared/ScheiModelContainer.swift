...
enum ScheiModelContainer {
    ...
    private static let storeFileName = "Schei.store"
    ...
    static func make(inMemory: Bool = false) -> ModelContainer {
        ...
        do {
            ...
            print("[Schei] Persistent store unavailable (\(error)); falling back to in-memory.")
    ...
    private static func migrate(from legacy: URL, to shared: URL) -> Bool {
        ...
        for suffix in ["", "-wal", "-shm"] {
            ...
            do {
                ...
                print("[Schei] Store migration failed for \(source.lastPathComponent): \(error)")
                discardPartialCopy()
                return false
            }
        }
        ...
        print("[Schei] Store migrated into the App Group container.")
        return true
    }
