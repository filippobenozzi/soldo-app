...
struct AddExpenseView: View {
        ...
        Menu {
            ...
            HStack(spacing: 12) {
                Image(systemName: symbol)
                    .foregroundStyle(value == nil ? Color.secondary : ScheiTheme.tint(hex))
                    .frame(width: 22)
                Text(title)
        ...
        VStack(spacing: 0) {
            HStack(spacing: 12) {
                Image(systemName: place == nil ? "mappin.slash" : "mappin.circle.fill")
                    .foregroundStyle(place == nil ? .secondary : ScheiTheme.ink)
                    .frame(width: 22)
    ...
    private var locationHint: String {
        if locationService.isDenied {
            return "Accesso negato. Attivalo in Impostazioni iOS › Schei › Posizione."
        }
