...
extension View {
    ...
    func scheiSwitch() -> some View {
        tint(ScheiTheme.switchTint)
    }

    /// The rounded white surface used for every card in the app.
    func scheiCard(padding: CGFloat = ScheiTheme.cardPadding) -> some View {
        self
            .padding(padding)
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(
                RoundedRectangle(cornerRadius: ScheiTheme.cornerRadius, style: .continuous)
...
struct SymbolBadge: View {
    ...
    var body: some View {
        Image(systemName: symbolName)
            ...
            .foregroundStyle(ScheiTheme.tint(colorHex))
...
struct BudgetRing: View {
    ...
    private var tint: Color {
        progress > 1 ? ScheiTheme.danger : ScheiTheme.ink
    }

    var body: some View {
        ZStack {
            Circle()
                .stroke(ScheiTheme.badge, lineWidth: lineWidth)
            Circle()
        ...
        configuration.label
            ...
            .background(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .fill(ScheiTheme.ink.opacity(configuration.isPressed ? 0.75 : 1))
            )
